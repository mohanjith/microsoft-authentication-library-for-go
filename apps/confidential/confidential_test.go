...
func TestRefreshIn(t *testing.T) {
	...
	for _, needsRefresh := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			mockClient.AppendResponse(
				mock.WithBody([]byte(fmt.Sprintf(`{"access_token":%q,"expires_in":3600,"refresh_in":%d,"token_type":"Bearer"}`, secondToken, refreshIn))),
			)
			...
			for i := 0; i < 100; i++ {
				...
				go func() {
					...
					// call, which will panic when more than one goroutine attempts a refresh.
